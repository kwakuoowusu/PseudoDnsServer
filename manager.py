...
def manage(rend):
	...
	serverSocket.bind(('allv24.all.cs.stonybrook.edu',serverPort))
	...
	while True: 
		...
		t = Thread(target=connectionHandler, args=(connectionSocket,serverSocket,ipAddr))
		t.start()


#handles connections for multiple usrs
def connectionHandler(connectionSocket,serverSocket,ipAddr):
	while True:
		...
		# browse command to list all types suported by manager
			...
			# catch if ctrl-c pressed by client 
			except IOError:
				print "Client at IP address %s disconnected" %ipAddr
				exit(0)
			continue
...
def list_type(connectionSocket):
	...
	server3 = all_types[2].split()
	server4 = all_types[3].split()
...
def request_type(server_type, connectionSocket):
	...
	elif(server_type.upper() == server3[0].upper()): #if the type is handled by server2
		connectionSocket.send("613 TYF " + ipAddr + " " + server3[1])

	elif(server_type.upper() == server4[0].upper()): #if the type is handled by server2
		connectionSocket.send("613 TYF " + ipAddr + " " + server4[1])
...
if __name__ == '__main__':
	...
	#try catch to get ctrl-c from client
	try:
		main()
		...
		print "Port cooldown, please wait"
		exit(0)